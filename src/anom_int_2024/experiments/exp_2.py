...
class EXP_2(EXP_BASE):

    def __init__(self, rng, win_feedb):
        super().__init__('EXP_2', rng, win_feedb)
    ...
    def feedb_pvalue(self, frame_i, exp_data_dict, bytes_array):
        # Invert bytes on session's second half
        frame_n = exp_data_dict['frame_n']
        if frame_i >= frame_n//2:
            bytes_array = np.invert(bytes_array)
